...
class Trainer(TrainerBase):
    def __init__(self, cfg):
        ...
        self.logger = get_root_logger(
            log_file=os.path.join(cfg.save_path, "log_train.txt"),
            file_mode="a" if cfg.resume else "w",
        )
        self.logger.info("=> Loading config ...")
        self.cfg = cfg
        self.logger.info(f"Env: {sys.executable}, torch-{torch.__version__}-cu{torch.version.cuda}")
        ...
        if pretty_text:
            ...
            with redirect_io(log_file=self.logger):
                log_config(cfg)
    ...
    def after_epoch(self):
        ...
        for i in range(torch.cuda.device_count()):
            ...
            used = total - free
            info += f' GPU{i}: [{used / 1024 ** 3:.1f}/{total / 1024 ** 3:.1f}]'
    ...
    def build_model(self):
        ...
        model = create_ddp_model(
            model.cuda(),
            # avoid sync (broadcast) the buffers at each forward - inplace operations, affect mostly Batch/Instance-Norm layers
    ...
    def get_lora_model(self, model):
        if not self.cfg.resume:
            self.load_checkpoint(model, **self.cfg.lora.pop("load", {}))  # load original ckpt before lora
        from pointcept.models.blocks.peft import inject_adapters
        model = inject_adapters(model, lora_config=self.cfg.lora)
        return model

    def load_checkpoint(self, model, keywords="", replacement="", skip=None, strict=False):
        # load checkpoint (before DDP)
        self.logger.info("=> Loading checkpoint & weight ...")
        if self.cfg.weight:
            assert os.path.isfile(self.cfg.weight), f'weight not found: {self.cfg.weight}'
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                if skip and skip.search(key):
                    ...
                    continue
                weight[key] = value
            missing_keys, unexpected_keys = model.load_state_dict(weight, strict=strict)
    ...
    def build_writer(self):
        enable_summary = comm.is_main_process() and hasattr(self.cfg, 'enable_summary') and self.cfg.enable_summary
        writer = SummaryWriter(self.cfg.save_path) if enable_summary else None
    ...
    def build_scheduler(self):
        ...
        self.cfg.scheduler.total_steps = len(self.train_loader) * self.cfg.eval_epoch // self.cfg.gradient_accumulation_steps
