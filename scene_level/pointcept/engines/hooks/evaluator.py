...
class SemSegEvaluator(HookBase):
    def __init__(self, write_cls_iou=False, verbose=2):
        self.write_cls_iou = write_cls_iou
        self.verbose = verbose
        assert verbose in [1, 2]
    ...
    def eval(self):
        ...
        if self.verbose > 1:
            ...
            if self.trainer.cfg.data.val.type in ["ScanNet200Dataset"]:
                # calc on head-common-tail classes
                cats_masks = self.trainer.val_loader.dataset.get_cats_mask()
                met = dict(mIoU=m_iou, IoUs=[], mACC=m_acc, ACCs=[], OA=all_acc)
                for cats_mask in cats_masks.values():
                    # cats_mask = np.logical_and(cats_mask, mask)
                    met["IoUs"] += [np.mean(iou_class[cats_mask])]
                    met["ACCs"] += [np.mean(acc_class[cats_mask])]
                met = Metrics(met, names=tuple(cats_masks.keys()))
            else:
                met = dict(mIoU=m_iou, IoUs=iou_class, mACC=m_acc, ACCs=acc_class, OA=all_acc)
                met = Metrics(met, names=self.trainer.cfg.data.names)
            met.print(logger=self.trainer.logger)
