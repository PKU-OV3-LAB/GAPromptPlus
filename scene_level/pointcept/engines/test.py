...
class TesterBase:
    def __init__(
        self, cfg, model=None, test_loader=None, logger=None, verbose=0,
        save=False, fragment_batch_size=None
    ) -> None:
        torch.multiprocessing.set_sharing_strategy("file_system")
        if logger is None:
            logger = get_root_logger(
                log_file=os.path.join(cfg.save_path, "log_test.txt"),
        ...
        self.save = save
        self.fragment_batch_size = fragment_batch_size
        self.hooks = []
        ...
        if self.verbose:
            ...
            if pretty_text:
                ...
                from utils.logger import redirect_io
                with redirect_io(log_file=logger, debug=cfg.debug):
                    log_config(cfg)
        ...
        if cfg.test.hooks:
            self.register_hooks(cfg.test.hooks)
        return

    def register_hooks(self, hooks):
        import weakref
        from .hooks import HookRecorder, build_hooks

        hooks = [dict(type=h) if isinstance(h, str) else h for h in hooks]  # simple conversion - easier comand-line setting
        hooks = build_hooks(hooks)
        for h in hooks:
            assert isinstance(h, HookRecorder), f"can only use hooks for tester, but get {h.__class__}"
            h.tester = weakref.proxy(self)  # weak-ref
    ...
    def build_model(self):
        ...
        if os.path.isfile(self.cfg.weight):
            if self.verbose:
                path = self.cfg.weight
                if os.path.islink(path):
                    path = f"{path} -> {os.path.relpath(os.path.realpath(path))}"
                self.logger.info(f"Loading weight at: {path}")
            checkpoint = torch.load(self.cfg.weight, weights_only=False)
            skip = self.cfg.weight_skip
            skip = re.compile(skip if isinstance(skip, str) else "|".join(skip)) if skip else False
            weight = OrderedDict()
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                if skip and skip.search(key):
                    weight_skipped[key] = value
                    continue
                weight[key] = value
            has_backbone_prefix = any(
                key.startswith(("backbone.", "module.backbone."))
                for key in weight
            )
            if has_backbone_prefix:
                ...
                self.logger.info("=== Load full finetuned checkpoint ===")
            else:
                backbone = (
                ...
                self.logger.info("=== Load pretrain backbone-only checkpoint ===")
            msg_missing = "\n\t".join(["Missing keys:", *missing_keys])
            msg_unexpected = "\n\t".join(["Unexpected keys:", *unexpected_keys])
            msg_skipped = "\n\t".join(["Skipped keys:", *weight_skipped.keys()])
            assert not missing_keys or set(missing_keys) == set(weight_skipped.keys()), f"{msg_missing}\nwhile {msg_skipped}"
            assert not unexpected_keys, f"{msg_unexpected}\nwhile {msg_skipped}"
            if self.verbose:
                self.logger.info("=> Loaded weight '{}' (epoch {})".format(self.cfg.weight, checkpoint["epoch"]))
                if weight_skipped:
                    self.logger.info(msg_skipped)
        else:
            raise RuntimeError("=> No checkpoint found at '{}'".format(self.cfg.weight))
        return model

    def get_lora_model(self, model):
        from pointcept.models.blocks.peft import inject_adapters
        model = inject_adapters(model, lora_config=self.cfg.lora)
        return model
    ...
    def build_test_loader(self):
        ...
        if comm.get_world_size() > 1:
            ...
            test_sampler = None
        num_worker = self.cfg.test.num_worker if isinstance(self.cfg.test.num_worker, int) else self.cfg.batch_size_test_per_gpu
        if isinstance(self.cfg.test.use_shared_memory, bool):
            from torch.utils.data._utils.collate import default_collate
            default_collate._use_shared_memory = self.cfg.test.use_shared_memory
        test_loader = torch.utils.data.DataLoader(
            ...
            pin_memory_device=self.cfg.pin_memory_device or 'cuda',
...
class SemSegTester(TesterBase):
    def test(self, logger=None, verbose=None, save=None):
        if logger is None:
            logger = self.logger
        if verbose is None:
        ...
        if save is True:
            ...
            save = tuple([i for i in save.split("-") if i])
        elif save:
            assert isinstance(save, (list, tuple)), f"unexpected save={save}"
        ...
        test_model = re.split(r"[_-]", os.path.basename(self.cfg.weight))[-1].split('.')[0]
        save_path = self.cfg.test.save_path if self.cfg.test.save_path else os.path.join(self.cfg.save_path, f"result_{test_model}")
        self.cfg.test.save_path = save_path
        if save and comm.is_main_process():
            # create folders only on main process
            make_dirs(save_path)
            if "submit" in save:
                # .../Log dir/result/submit
                if self.cfg.data.test.type in ["ScanNetDataset", "ScanNet200Dataset", "ScanNetPPDataset"]:
        ...
        # result dict: scene (data name) -> {intersection, union, target (gt)}
        record = {}
        for h in self.hooks:
            h.record_init(record)

        empty_cache = self.cfg.empty_cache or self.cfg.test.empty_cache
        empty_cache_per_epoch = self.cfg.empty_cache_per_epoch or self.cfg.test.empty_cache_per_epoch
        if isinstance(self.cfg.test.use_shared_memory, bool):
            from torch.utils.data._utils.collate import default_collate
            default_collate._use_shared_memory = self.cfg.test.use_shared_memory
            from pointcept.datasets.utils import default_collate as default_collate
            default_collate._use_shared_memory = self.cfg.test.use_shared_memory
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            fragment_list = data_dict.pop("fragment_list")  # clouds of diff augmentations covering the current scene
            ...
            if os.path.isfile(pred_save_path):
                if verbose > 1:
                    logger.info(
                        "{iter:>{width}}/{max_iter}: {name}, loaded pred and label.".format(
                            iter=idx+1, width=len(str(max_iter)), max_iter=max_iter, name=data_name
                        )
                    )
                ...
                fragment_batch_size = self.cfg.test.fragment_batch_size if self.cfg.test.fragment_batch_size else 1
                fragment_batch_num = np.ceil(len(fragment_list) / fragment_batch_size).astype(int)
                for i in range(fragment_batch_num):
                    ...
                    with torch.inference_mode():
                        ...
                        for be in input_dict["offset"]:
                            ...
                            bs = be
                    for h in self.hooks:
                        h.record_sample(data_name, segment.size, input_dict, pred_part, record)

                    if verbose > 2:
                        logger.info(
                            "Test: {:>{width_i}}/{}-{data_name}, Batch: {batch_idx:>{width_b}}/{batch_num}".format(
                                idx + 1,
                                max_iter,
                                width_i=len(str(max_iter)),
                                data_name=data_name,
                                batch_idx=i,
                                batch_num=fragment_batch_num,
                                width_b=len(str(fragment_batch_num)),
                            )
                        )
            ...
            result_dict = dict(
                pred=pred, segment=segment
            )
            for h in self.hooks:
                h.record_cloud(data_name, data_dict, result_dict, record)
        ...
        if comm.get_world_size() > 1:
            stats = torch.as_tensor(
                np.stack([intersection, union, target]),
                dtype=torch.float64,
                device="cuda",
            )
            torch.distributed.all_reduce(stats)
            intersection, union, target = stats.cpu().numpy()

        record_sync = comm.gather(record, dst=0) if self.hooks else None
        if comm.is_main_process():
            if record_sync is not None:
                record = {}
                for rank_record in record_sync:
                    record.update(rank_record)
            else:
                record = {}

            # if self.cfg.data.test.type == "S3DISDataset" and save:
            #     torch.save(
            ...
            met = dict(mIoU=mIoU, IoUs=iou_class, mACC=mAcc, ACCs=accuracy_class, OA=allAcc)

            for h in self.hooks:
                h.record_dataset(record)
            ...
            if verbose > 1:
                ...
                if self.cfg.data.test.type in ["ScanNet200Dataset"]:
                    # calc on head-common-tail classes
                    cats_masks = self.test_loader.dataset.get_cats_mask()
                    cats_met = dict(mIoU=mIoU, IoUs=[], mACC=mAcc, ACCs=[], OA=allAcc)
                    for cats_mask in cats_masks.values():
                        cats_mask = np.logical_and(cats_mask, mask)
                        cats_met["IoUs"] += [np.mean(iou_class[cats_mask])]
                        cats_met["ACCs"] += [np.mean(accuracy_class[cats_mask])]
                    met = Metrics(met, names=None)
                    cats_met = Metrics(cats_met, names=tuple(cats_masks.keys()))
                    if verbose:
                        logger.info("")
                        met.print(logger=logger)
                        cats_met.print(logger=logger)

                elif self.cfg.data.test.type in ["S3DISDataset"]:
                    met.update(conf=dict(intersection=intersection, union=union, target=target))  # for 6-fold - intersection (TP), union (TP+FN+FP), target (TP+FN)
                    met = Metrics(met, names=self.cfg.data.names)
                    if verbose:
                        logger.info("")
                        met.print(logger=logger)
            ...
            if verbose > 1:
                # logger.info(f"elapsed time: {(time.time() - _eval_start) / 60:.1f}min")
                logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
            if verbose:
                logger.info(f"elapsed_time: {(time.time() - _eval_start) / 60 ** 2:.1f}h")
                logger.info("\nfinished\n")
            return met
