    ...
    def forward(self, input_dict, return_point=False):
        ...
        if isinstance(point, Point):
            while "pooling_parent" in point.keys():
                # - aggregate all feat from encoder-only model (pooled points)
                # - no influence on PTv3 (as w/ dec)
                # TODO: a more elegant & configurable way?
...
class FinetuneSegmentorV2(nn.Module):
    def __init__(
        ...
        if self.freeze_backbone:
            if self.activate_param:
                for name, p in self.backbone.named_parameters():
                    if any([param.strip() in name for param in self.activate_param.split(',')]):
