...
from .pointgst import PCSA, sort, get_basis
...
class Group(nn.Module):
    ...
    def forward(self, point):
        ...
        batch = point.batch  # [N]
        # FPS
        center_indices = fps(xyz, batch=batch, max_num_samples=self.num_group)
        center_points = xyz[center_indices]
        center_batch = batch[center_indices] if batch is not None else torch.zeros(center_indices.size(0),
                                                                                   dtype=torch.long, device=xyz.device)
        col, row = knn(xyz, center_points, k=self.group_size, batch_x=batch, batch_y=center_batch)
        neighborhood_points = xyz[col]
        batch_size = batch.max().item() + 1 if batch is not None else 1
        neighborhood = neighborhood_points.view(batch_size, self.num_group, self.group_size, 3)
        center = center_points.view(batch_size, self.num_group, 3)
        # norm
...
class Block(PointModule):
    def __init__(
        ...
        if self.is_dec is not True:
            self.adapter = PCSA(dim=channels, rank=64)


    def forward(self, point: Point, U=None, sub=None, idx=None, counts=None, batch=None):
        shortcut = point.feat
        ...
        if self.is_dec:
            ...
            shortcut = point.feat + self.adapter(point.feat, U, sub, idx, counts, batch)
        if self.pre_norm:
...
@MODELS.register_module("PT-v3m2-pgst")
class PointTransformerV3(PointModule):
    def __init__(
        ...
        for s in range(self.num_stages):
            ...
            enc = PointSequential()
            enc.add(
                Group(num_group=self.group_num, group_size=self.group_size,),
                name="group"
            )
            if s > 0:
                enc.add(
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.num_stages):
            stage_module = getattr(self.enc, f"enc{i}")
            group = getattr(stage_module, "group")
            neighbour, center = group(point)
            U = get_basis(center)
            B, G, _ = center.shape
            c = center * 100
            ...
            # Build sub-group basis in a shape-safe way.
            # PCSA requires: sub_group_num * sub_group_size == G (so that U @ x matches).
            # The original code hard-coded sub_group_size=32 which breaks when G is not a multiple of 32
            # or when G < 32. Here we choose the largest divisor <= 32.
            max_sub_group_size = min(32, int(G))
            sub_group_size = None
            for d in range(max_sub_group_size, 0, -1):
                if int(G) % d == 0:
                    sub_group_size = d
                    break
            if sub_group_size is None:
                sub_group_size = 1
            sub_group_num = int(G) // sub_group_size

            sub_U = get_basis(sub_center.reshape(B * sub_group_num, sub_group_size, 3)).reshape(B, sub_group_num, sub_group_size, sub_group_size)
            if i > 0:
                down = getattr(stage_module, "down")
                point = down(point)
            batch = point.batch
            counts = torch.bincount(batch)
            for j in range(self.enc_depths[i]):
                block = getattr(stage_module, f"block{j}")
                point = block(point, U=U, sub=sub_U, idx=[idx0, idx1], counts=counts, batch=batch)
