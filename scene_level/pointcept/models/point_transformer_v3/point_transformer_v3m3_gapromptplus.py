...
@MODELS.register_module("PT-v3m3-gapromptplus")
class PointTransformerV3(PointModule):
    def __init__(
        ...
        self.point_shift_prompter = ShiftNet(3, 3, enc_channels[0])
        ...
        for s in range(self.num_stages):
            ...
            if s > 0:
                ...
                if s in self.prompt_stages:
                    ...
                    point.sparsify()
                    point = prompt_propagation(point, prompt_indexs=prompt_indexs, k=32, fusion_ratio=0.02)

            for i in range(self.enc_depths[s]):
                block = getattr(stage, f"block{i}")
                point = block(point, non_linear=self.enc_nonlinar_mappings[f'enc{s}-nonlinar{i}'])

        if self.prompt_stages:
            point = remove_prompt(point)
        if not self.enc_mode:
...
class ShiftNet(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_dimension=384, perturbation=0.00001):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
    ...
    def forward(self, point: Point):
        shift_prompt = self.mlp_position(point.feat) * self.perturbation
...
def remove_prompt(point: Point, prompt_mark='prompt_point_number'):
    ...
    for i in range(hierarchies):
        prompt_number = cur[prompt_mark] if prompt_mark in cur.keys() else 0
        if prompt_number==0:
            prev = cur
...
def propagate(xyz1, xyz2, feat2, fusion_ratio = 0.01):
    """
    Input:
