...
class SerializedAttention(PointModule):
    def __init__(
        self,
        channels,
        # channels=enc_channels[s],     # (48, 96, 192, 384, 512),
        num_heads,
        # num_heads=enc_num_head[s],    # (3, 6, 12, 24, 32),
        patch_size,
        ...
        self.stage = stage
        self.embed_adap = None
        if embed_adap is not None:
...
from .point_transformer_v3m1_base import SerializedAttention as SerializedAttention_base
SerializedAttention_sonata = SerializedAttention
SerializedAttention = SerializedAttention_base
...
class Block(PointModule):
    ...
    def forward(self, point: Point):
        ...
        if self.embed_pattsc is not None:  # parallel to attn-sc
            feat = self.drop_path(self.ls1(self.attn(point))).feat
            point = self.embed_pattsc(Point(point, feat=shortcut))
            point.feat = point.feat + feat
        elif self.embed_pstatt is not None:  # post-attn (before sc)
            point = self.drop_path(self.ls1(self.embed_pstatt(self.attn(point))))
            point.feat = shortcut + point.feat
        else:
...
class GridPooling(PointModule):
    ...
    def forward(self, point: Point):
        ...
        # points (grid_coord as keys) with same higher digits & diff lower digits (in same grid) pooled into 1
        ...
        if "eigfeat" in point.keys():
            point_dict["eigfeat"] = torch_scatter.segment_csr(point.eigfeat[indices], idx_ptr, reduce="mean")
...
class GridUnpooling(PointModule):
    def __init__(
        ...
        unpool_keys=None,  # keys for pooling_parent update
    ):
        super().__init__()
...
@MODELS.register_module("PT-v3m2-gem")
class PointTransformerV3(PointModule):
    def __init__(
        ...
        # stem: lin - norm (ln) - act (gelu)
        self.embedding = Embedding(
            in_channels=in_channels,
            embed_channels=enc_channels[0],
            # use ln-layer everywhere
            norm_layer=ln_layer,
            act_layer=act_layer,
