...
class PCSA(nn.Module):
    def __init__(self, dim, rank):#cfg
        super().__init__()
    ...
    def forward(self, input, U, sub_U, idx, counts, batch):
        input = scatter_points_to_batch(input, batch).permute(0, 2, 1).contiguous()
        h = self.down(input)
        ...
        h = self.up(h)
        h = h * self.scale
        h = restore_points_from_batch(h.permute(0, 2, 1).contiguous(), counts)
        return h
