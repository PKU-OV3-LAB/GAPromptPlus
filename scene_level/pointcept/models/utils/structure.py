...
class Point(Dict):
    ...
    def numpy(self, inplace=True):
        container = self if inplace else Point()
        for k, v in self.items():
            if isinstance(v, torch.Tensor):
                container[k] = v.detach().cpu().numpy()
            elif not inplace:
                container[k] = v
        return container

    def cuda(self, inplace=True, non_blocking=True):
        container = self if inplace else Point()
        for k, v in self.items():
            if isinstance(v, torch.Tensor):
                container[k] = v.cuda(non_blocking=non_blocking)
            elif isinstance(v, np.ndarray):
                container[k] = torch.from_numpy(v).cuda(non_blocking=non_blocking)
            elif not inplace:
                container[k] = v
        return container
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        """
        self["order"] = order
        self["serialized_order_index"] = torch.arange(len(order), device=self["coord"].device)
        assert "batch" in self.keys()
        if "grid_coord" not in self.keys():
            # if you don't want to operate GridSample in data augmentation,
        ...
        # - [bits for batch inds, bits for max xyz-coord]
        ...
        # [k, N] - per-point keys (for each of the k serialization): point inds -> serialization keys
        code = torch.stack(code)
        # - sorted inds (into serialization) according to keys: point inds -> serialization inds
        order = torch.argsort(code)
        # [k, N] - serialization inds -> point inds
        ...
        if shuffle_orders:
            ...
            self["serialized_order_index"] = self["serialized_order_index"][perm]
    ...
    def sparsify(self, pad=96):
        ...
        sparse_conv_feat = spconv.SparseConvTensor(
            features=self.feat,
            # indices - [N, ndims+1] - from cat[B_inds, xyz(ndims=3)]
            indices=torch.cat(
