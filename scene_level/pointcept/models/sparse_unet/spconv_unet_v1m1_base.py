...
try:
    from pointcept.models.blocks.encoding import SparsePointHook
    sparse_point_hook = SparsePointHook(
        keys=("latents",),
    )
except:
    sparse_point_hook = None

# class SparseConvPoint(Point, spconv.SparseConvTensor):
...
#     @property
#     def feat(self):
#         feat = self.__dict.get("feat", self.features)
#         return feat
#     @feat.setter
#     def feat(self, f):
#         self.__dict["feat"] = f
...
#         new_spt = SparseConvPoint(feature, self.indices, self.spatial_shape,
...
class BasicBlock(spconv.SparseModule):
    ...
    def __init__(
        ...
        from pointcept.models.blocks.encoding import build_encoding_spunet
        _kwargs = dict(
            ...
            hook=sparse_point_hook,
        )
        _build_embed = partial(build_encoding_spunet, **_kwargs)
        ...
        self.relu = nn.ReLU()
        self.embed_pos = _build_embed(embed_pos)        # after enc
        self.embed_ppos = _build_embed(embed_ppos)      # parallel enc
    ...
    def forward(self, x):
        ...
        if self.embed_ppos is not None:  # parallel to enc
            out = self.conv1(x)
            out = out.replace_feature(self.bn1(out.features))
            out = out.replace_feature(self.relu(out.features))
            out = out.replace_feature(out.features + self.embed_ppos(x).features)
        else:
            out = self.conv1(x)
            out = out.replace_feature(self.bn1(out.features))
            out = out.replace_feature(self.relu(out.features))
        if self.embed_pos is not None:
            out = self.embed_pos(out)

        if self.embed_pattsc is not None:  # parallel to attn-sc
            feat = self.embed_pattsc(out, shortcut=self.proj(residual).features)

            out = self.conv2(out)
            out = out.replace_feature(self.bn2(out.features))

            out = out.replace_feature(out.features + feat.features)
            out = out.replace_feature(self.relu(out.features))
