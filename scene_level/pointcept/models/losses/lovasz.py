...
class LovaszLoss(_Loss):
    def __init__(
        ...
        NOTE:
            directly optimize the IoU/mIoU
                - by using its convex closure, the Lovasz extension of mIoU (as a set function, shown to be submodular)
                - Lovasz extension neet to sort the set element (pixel/point) through a surrogate (per-pixel/point loss)

            lovasz hinge:
                - using hingle loss (per-class 0-1 gt)
                => binary/multilabel

            lovasz softmax:
                - using gap in probs: (1 - gt) * prob + gt * (1 - prob)
                - for lovasz-IoU loss on each class, as per-class IoU, then mean over class, as mIoU
                => multiclass

            TODO: using cross-entropy ?
                - as a special weighting, naturally combined to cross-entropy loss?

            compare with Dice loss:
                - focus on class imbalance (instead of directly optimizing IoU/mIoU)
                - get instable gradient for unbalanced class with small population (tail class)
        """
