...
class PointPromptTraining(nn.Module):
    ...
    def __init__(
        ...
        if class_names is None:
            # fmt: off
            class_names = [
                # classes of each dataset (in `conditions`)
        ...
        if not self.backbone_mode:
            ...
            for i in range(len(conditions)):
                class_prompt = [template.replace("[x]", name) for name in class_names[i]]
                class_token = clip.tokenize(class_prompt)
                # [ncls_i, embed_dim (d_clip)]
    ...
    def forward(self, data_dict):
        ...
        sim = (
            ...
            @ self.class_embeddings.split(self.num_classes)[
                # split & index the class embeddings of current dataset
