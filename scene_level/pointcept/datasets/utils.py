    ...
    if random.random() < mix_prob:
        if "instance" in batch.keys():
            ...
            for i in range(len(offset)):
                if i % 2 == 0:
                    # record the max instance-id in the 1st cloud
                    num_instance = max(batch["instance"][start : offset[i]])
                if i % 2 != 0:
                    # shift the instance-id in the 2nd cloud - to be non-overlap with the 1st cloud
                    mask = batch["instance"][start : offset[i]] != -1
                    batch["instance"][start : offset[i]] += num_instance * mask
                start = offset[i]
        if "offset" in batch.keys():
            # mixing every 2-clouds - Mix3d (https://arxiv.org/pdf/2110.02210.pdf)
            batch["offset"] = torch.cat(
                # NOTE: offset = right-bound inds for each example (sampled cloud) - i-th cloud ranges from to `offset[i-1]` to `offset[i]`
                # => mixing each neighboring cloud, till the last
