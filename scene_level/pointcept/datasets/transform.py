...
def index_operator(data_dict, index, duplicate=False):
    ...
    if "index_valid_keys" not in data_dict:
        data_dict["index_valid_keys"] = list(_index_valid_keys)
    if not duplicate:
...
@TRANSFORMS.register_module()
class CollectNumpy(object):
    def __init__(self, keys, offset_keys_dict=None, **kwargs):
        """
        e.g. CollectNumpy(keys=[coord], feat_keys=[coord, color])
        """
        if offset_keys_dict is None:
            offset_keys_dict = dict(offset="coord")
        self.keys = keys
    ...
    def __call__(self, data_dict):
        ...
        for key in self.keys:
            data[key] = data_dict[key]
        for key, value in self.offset_keys.items():
            data[key] = np.array([data_dict[value].shape[0]])
        for name, keys in self.kwargs.items():
            name = name.replace("_keys", "")
            assert isinstance(keys, Sequence)
            data[name] = np.cat([data_dict[key] for key in keys], dim=1, dtype=float)
        return data
...
class CenterShift(object):
    def __init__(self, apply_z=True):
        # x-y centering, 0-grounding for z-axis (if `apply_z`)
    ...
    def __init__(self, shift=((-0.2, 0.2), (-0.2, 0.2), (0, 0))):
        ...
        for rng in shift:
            if isinstance(rng, Sequence):
                ...
                raise ValueError(f'not support rng={rng} with type={type(rng)}')
            assert len(rng) == 2 and all(isinstance(i, float) for i in rng)
            self.shift.append(rng)
...
class ElasticDistortion(object):
    ...
    def elastic_distortion(coords, granularity, magnitude):
        ...
        try:
            ...
            msg = (
                f"granularity={granularity}, magnitude={magnitude}\n"
                f"noise_dim={noise_dim}; dist(coords)={(coords - coords_min).max(0)}; dist(grid)={((coords - coords_min).max(0) // granularity)}"
            )
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        # inverse   : [N]       - indices to reconstruct key_sort, indexing into unique key `_`
        # count     : [#unq]    - count of repeated times for each unique key - #pts in the same grid cell
        if self.mode == "train":  # train mode
            idx_select = (
                # cumsum to start with 0 - starting idx of each unique key
                np.cumsum(np.insert(count, 0, 0)[0:-1])
                # random pick 1pt per cell
            ...
            for k in self.keys:
                # additional keys
                if k not in data_dict["index_valid_keys"]:
                    data_dict[k] = data_dict[k][idx_unique]
                    data_dict["index_valid_keys"].append(k)
            if self.return_inverse:
                data_dict["inverse"] = np.zeros_like(inverse)
                # indices indexing into unique key, stored back to original grid order
                # [N] - voting from select pts to all original pts (within each grid)
                data_dict["inverse"][idx_sort] = inverse
            if self.return_grid_coord:
                # grid_coord, unique & complete
            ...
            if self.return_displacement:
                displacement = (
                    ...
                    # acutally [0, 0] -> [0.5, 0.5]
                )  # [0, 1] -> [-0.5, 0.5] displacement to center
                if self.project_displacement:
                    displacement = np.sum(
                        displacement * data_dict["normal"], axis=-1, keepdims=True
                    )
                # displacement of select pts to the cell center
            ...
            for i in range(count.max()):
                # repeat selection, to iterate at least once for each pts in a cell (capped by the densest cell)
                ...
                if self.return_displacement:
                    ...
                    if self.project_displacement:
                        displacement = np.sum(
                            displacement * data_part["normal"], axis=-1, keepdims=True
                        )
...
class SphereCrop(object):
    def __init__(self, point_max=80000, sample_rate=None, mode="random"):
        ...
        assert mode in ["random", "center", "all", "all_cnt"]
        self.mode = mode
    ...
    def __call__(self, data_dict):
        ...
        assert "coord" in data_dict.keys()
        if self.mode in ["all", "all_cnt"]:
            # repeat sphere-cropping to cover the whole scene, based on tukey potentials - spatially_regular_gen
            # TODO: Optimize
            ...
            if data_dict["coord"].shape[0] > point_max and self.mode == "all":
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    data_crop_dict = index_operator(data_dict, idx_crop, duplicate=True)
            ...
            elif data_dict["coord"].shape[0] > point_max and self.mode == "all_cnt":
                coord_p = np.random.rand(data_dict["coord"].shape[0]) * 1e-3
                idx_cnt = np.zeros(data_dict["coord"].shape[0], dtype=np.int32)
                while idx_cnt.min() == 0:
                    init_idx = np.argmin(coord_p)
                    dist2 = np.sum(
                        np.power(data_dict["coord"] - data_dict["coord"][init_idx], 2),
        ...
        # mode is "random" or "center"
            ...
            if self.mode == "random":
                # select random pts as center
                center = data_dict["coord"][
                    np.random.randint(data_dict["coord"].shape[0])
                ]
            elif self.mode == "center":
                # use the mid index as center, actually still random center
