...
class PointTransformerDINO(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.encoder = Point_PN(k_neighbors=64, dim_expansion=config.get('encoder_dim_expansion', [2,2,2,1])) # [2,1,2,1]: 384; [2,2,2,1]: 768;
        ...
        if bert_ckpt_path is not None:
            ...
            for k in list(base_ckpt.keys()):
                if k.startswith('MAE_encoder') :
                    ...
                    del base_ckpt[k]
                elif k.startswith('transformer_q') and not k.startswith('transformer_q.cls_head'):
    ...
    def forward(self, pts):
        x = pts.transpose(1, 2).contiguous()
