...
from models.GapGPT import GPT_extractor, GPT_generator
import math
from models.z_order import *
from .GAPrompt_PointGPT import ShiftNet, PointPrompt, Group2, propagate, pooling
...
class Gap_PointTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        ...
        self.level2_group_divider = Group2(num_group=self.num_group//2, group_size=self.group_size//2)
        assert self.encoder_dims in [384, 768, 1024]
        if self.encoder_dims == 384:
            self.encoder = Encoder_small(encoder_channel=self.encoder_dims)
        else:
        ...
        self.point_prompt = PointPrompt(point_number=config.point_number, init_type='uniform', scale=0.01, factor=5)
        self.shift_net = None
        self.shift_net = ShiftNet(3,3, hidden_dimesion=self.encoder_dims, perturbation=config.perturbation, num_group=config.num_group, group_size=config.group_size)
    ...
    def forward(self, pts):
        ...
        # attn_mask = torch.full(
        #     (L+2+10, L+2+10), -float("Inf"), device=group_input_tokens.device, dtype=group_input_tokens.dtype
        # ).to(torch.bool)
        # attn_mask = torch.triu(attn_mask, diagonal=1)

        # transformer
        ret, encoded_features = self.blocks(x, pos, None, classify=True, propagation_dict=propagation_dict, shape_feature=shape_feature)
