...
from .PointPrompt import ShiftNet, PointPrompt, InstancePointPrompter, Group, propagate, pooling
from .Point_MAE_segment import PointNetFeaturePropagation, Encoder, Mlp, Attention
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if use_prompt and num_tokens:
            self.prompt_dropout = nn.Dropout(0.1)
            self.num_tokens = num_tokens
            self.prompt_embeddings = nn.Parameter(torch.zeros(self.num_tokens, dim))
        self.adapter=None
        if use_adapter:
            self.adapter = Adapter(embed_dims=dim, reduction_dims=self.config.adapter_config.adapter_dim)
    ...
    def forward(self, x, global_feature=None, multi_scale_prompt=None, token_position=None, layer_id=None, level1_center=None, level1_index=None, level2_center=None, level2_index=None, batch_idx=None):
        if global_feature is not None and multi_scale_prompt is not None and self.prompt_embeddings is not None:
            ...
            token_prompt = token_prompt+0.5*(multi_scale_prompt.mean(dim=1, keepdim=True)+global_feature)+token_position.expand(x.shape[0], -1, -1)
            x = torch.cat((token_prompt, x), 1)
        elif global_feature is not None and self.prompt_embeddings is not None:
            token_prompt = self.prompt_dropout(self.prompt_embeddings.expand(x.shape[0], -1, -1))
            token_prompt = token_prompt+0.5*global_feature+token_position.expand(x.shape[0], -1, -1)
            x = torch.cat((token_prompt, x), 1)
        ...
        if self.config.propagation_type == 'permutation_after_attention':
            ...
            vis_x = pooling(x_neighborhoods.reshape(B, level2_center.shape[1], -1, self.dim))+0.3*x_centers
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, global_feature=None, token_position=None, level1_center=None, level1_index=None, level2_center=None, level2_index=None, batch_idx=None, multi_scale_prompt=None):
        feature_list = []
        prompt_list = []
        fetch_idx = [3, 7, 11]
        ...
        for idx, block in enumerate(self.blocks):
            if idx in fetch_idx:
                prompt_after_attn, x = block(x + pos, global_feature=global_feature, token_position=token_position, layer_id=idx, level1_center=level1_center, level1_index=level1_index, level2_center=level2_center, level2_index=level2_index, multi_scale_prompt=multi_scale_prompt)
...
class PointTransformer_GAPromptPlus_seg(nn.Module):
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ckpt = torch.load(
                bert_ckpt_path, map_location="cpu", weights_only=False
            )
            ...
            for k in list(base_ckpt.keys()):
                if k.startswith('MAE_encoder') :
                    ...
                    del base_ckpt[k]
                elif k.startswith('PQAE_encoder'):
                    base_ckpt[k[len('PQAE_encoder.'):]] = base_ckpt[k]
                    del base_ckpt[k]
    ...
    def forward(self, pts, cls_label, contrast=False, target=None):
        B, N, _ = pts.shape
        shape_feature = None
        ...
        feature_list, prompt_list = self.blocks(x, pos, global_feature=shape_feature, multi_scale_prompt=multi_scale_prompt, token_position=token_pos)
        ...
        # x_global_feature = torch.cat((x_max_feature, prompt_feature, cls_label_feature), -1) #1152*2 + 128
        torch.cuda.empty_cache()

        if self.config.point_prompt:
            pts = pts[:,:-self.instance_point_prompter.point_number]
        ...
        if contrast:
            ...
            return x, 2*contrast_loss
        return x

class get_loss(nn.Module):
    def __init__(self, config):
        super(get_loss, self).__init__()
        self.contrast_loss = ContrastHead(config, nsample=[8, 8, 8, 16], nstride=8)
