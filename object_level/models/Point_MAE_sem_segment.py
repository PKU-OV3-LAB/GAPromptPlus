...
from .boundary_contrast import ContrastHead
...
def propagate(xyz1, xyz2, points1, points2, de_neighbors=64, mode='interpulate'):
    """
    Input:
...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        ...
        point_groups = point_groups.reshape(bs * g, n, _)
        # encoder
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, center=None):
        feature_list = []
        fetch_idx = [3, 7, 11]
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in fetch_idx:
                ...
                if center is not None:
                    ...
                    if i != fetch_idx[-1]: # not the last layer
                        new_center, fps_idx = misc.fps(center, center.shape[1]//2)
                        new_x = torch.gather(x, 1, fps_idx.long().reshape(x.shape[0], -1, 1).expand(-1,-1,x.shape[2]))
                        new_x = propagate(new_center, center, new_x, x, de_neighbors=6)
                        pos = torch.gather(pos, 1, fps_idx.long().reshape(pos.shape[0], -1, 1).expand(-1,-1,pos.shape[2]))
                        center = new_center
                        x = new_x
        if center is not None:
            return feature_list, center_list
        else:
            return feature_list

# segmentation finetune model
@MODELS.register_module()
class PointTransformer_sem_seg(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.input_features = config.get('input_features', ['pos'])
        self.input_features_dim = 0
        features_dim_dict = {'pos':3, 'x':3, 'heights':1}
        for key in features_dim_dict:
            if key in self.input_features:
                self.input_features_dim += features_dim_dict[key]

        self.group_divider = Group(num_group=self.num_group, group_size=self.group_size)

        self.encoder = Encoder(encoder_channel=self.encoder_dims, input_channel=self.input_features_dim)
        ...
        self.propagation_0 = PointNetFeaturePropagation(in_channel=384*3 + 3, mlp=[384*4, 1024], interpolate_neighbors=3)

        self.seg_head = nn.Sequential(
            nn.Conv1d(1024+384*6, 512, 1), #1024+384*6
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Conv1d(512, 256, 1),
    ...
    def forward(self, pts, features=None, contrast=False, target=None):

        B, N, _ = pts.shape
        # divide the point cloud in the same form. This is important
        neighborhood, center, neighborhood_idx, center_idx = self.group_divider(pts, require_index=True, gather_idx=True)
        if features is not None:
            if 'x' in self.input_features:
                neighborhood_x = torch.gather(features['x'], 1, neighborhood_idx.reshape(B, -1, 1).expand(-1,-1,3))
                neighborhood_x = neighborhood_x.view(B, self.num_group, self.group_size, 3).contiguous()
                neighborhood = torch.concat([neighborhood, neighborhood_x], dim=-1)
            if 'heights' in self.input_features:
                neighborhood_h = torch.gather(features['heights'], 1, neighborhood_idx.reshape(B, -1, 1))
                neighborhood_h = neighborhood_h.view(B, self.num_group, self.group_size, 1).contiguous()
                neighborhood = torch.concat([neighborhood, neighborhood_h], dim=-1)
        ...
        # feature_list, center_list = self.blocks(x, pos, center=center)
        ...
        if center_list is None:
            ...
            for i in range(len(feature_list)-1, -1, -1):
                if i!=0:
                    feature_list[i-1] = propagate(center_list[i-1], center_list[i], feature_list[i-1], feature_list[i], de_neighbors=6, mode='concatenate')
                else:
                    x = feature_list[i]
...
class get_loss(nn.Module):
    def __init__(self, config):
        ...
        self.contrast_loss = ContrastHead(config)
