...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if idx<self.config.prompt_depth:
    ...
    def forward(self, x, global_feature=None, multi_scale_prompt=None, token_position=None, layer_id=None, level1_center=None, level1_index=None, level2_center=None, level2_index=None, batch_idx=None):
        if global_feature is not None and multi_scale_prompt is not None and layer_id<self.config.prompt_depth:
        ...
        elif global_feature is not None and layer_id<self.config.prompt_depth:
            token_prompt = self.prompt_dropout(self.prompt_embeddings.unsqueeze(0).expand(x.shape[0], -1, -1))
            token_prompt = token_prompt + 0.5*global_feature.expand([-1, self.num_tokens, -1])+token_position.expand(x.shape[0], -1, -1)
            x = torch.cat((x[:,0:1], token_prompt, x[:,1:]), 1)
        ...
        #             x = x + self.adapter1(x+global_feature*0.5)
        #     else:
        #         x = x + self.adapter1(x)
    ...
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                num_tokens=config.num_tokens, idx=i,
                config=config
                )
            for i in range(depth)])

    def forward(self, x, pos, global_feature=None, token_position=None, level1_center=None, level1_index=None, level2_center=None, level2_index=None, batch_idx=None, feature_index=[11], patch_prompt=None, multi_scale_prompt=None):
        features = []
        for idx, block in enumerate(self.blocks):
            if patch_prompt is not None and self.config.prompt_depth:
                token_number = patch_prompt.shape[1]
                x[:,-token_number:] = x[:,-token_number:] + 0.2*patch_prompt
...
class PointTransformer_pointtokenprompt(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.prompt_cor = nn.Parameter(torch.zeros(self.config.num_tokens, 3))
        ...
        if config.shift_net == True:
            self.shift_net = ShiftNet(3, 3, hidden_dimension=config.encoder_dims, perturbation=config.perturbation, num_group=config.num_group, group_size=config.group_size)

        # self.local_geometry_prompter = PatchPrompter(in_channels=self.trans_dim, out_channels=self.trans_dim)
    ...
    def forward(self, pts, batch_idx=None):
        ...
        # transformer
        x = self.blocks(x, pos, global_feature = shape_feature, token_position = token_pos, level1_center=center, level1_index=level1_idx, level2_center=level2_center, level2_index=level2_idx, batch_idx=batch_idx, patch_prompt=None, multi_scale_prompt=multi_scale_prompt)
