...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    peft_list = ['point_prompt', 'shift_net', 'prompts', 'bnorm',
                 ...
                 'seg_head', 'label_conv', 'propagation_0',
                 'prompt_embeddings', 'prompt_cor']
    ...
    import shutil
    shutil.copy('models/gapromptplus_MAE_segment.py', str(args.experiment_path))
    # training
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for batch_idx, (points, label, target) in enumerate(tqdm(train_dataloader)):
            ...
            seg_pred, contrast_loss = classifier(points, cls_label, contrast=True, target=target)
            ...
            loss = criterion(seg_pred, target) + contrast_loss
            loss.backward()
...
def test_net(args, config, train_writer=None, val_writer=None):
    ...
    classifier.load_model_from_ckpt(args.ckpts)
    if args.use_gpu:
        classifier = classifier.cuda(args.local_rank)

    metrics = validate(
        logger, num_part, classifier, test_dataloader, num_classes, config
    )
    print_log(
        "[TEST] Accuracy: {:.6f} Class avg mIOU: {:.6f} "
        "Instance avg mIOU: {:.6f}".format(
            metrics["accuracy"] * 100,
            metrics["class_avg_iou"] * 100,
            metrics["inctance_avg_iou"] * 100,
        ),
        logger=logger,
    )
    return metrics
