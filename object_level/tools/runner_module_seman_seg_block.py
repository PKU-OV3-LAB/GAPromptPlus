...
from datasets.S3DISBlockDataset import S3DISBlockDataset
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    TRAIN_DATASET = S3DISBlockDataset(split='train', data_root=config.root, num_point=config.npoints, test_area=5)
    ...
    TEST_DATASET = S3DISBlockDataset(split='test', data_root=config.root, num_point=config.npoints, test_area=5)
    ...
    peft_list = ['point_prompt', 'shift_net',
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for batch_idx, (points, target) in enumerate(tqdm(train_dataloader)):
            ...
            seg_pred, contrast_loss = classifier(points, contrast=True, target=target)
