...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    peft_list = ['point_prompt', 'shift_net', 'prompts', 'bnorm',
                'instance_point_prompter', 'point_shift_prompter',
                'local_geometry_prompter', 'adapter',
                'shape_feature_mlp', 'cls_token',
                'cls_pos', 'cls_head_finetune',
                'prompt_embeddings', 'prompt_cor', 'out_transform']
    if config.get('encoder_tune'):
        peft_list += ['encoder', 'pos_embed']
    ...
    shutil.copy('tools/runner_module.py', args.experiment_path)
    shutil.copy('models/prompt_MAE.py', args.experiment_path)
    shutil.copy('models/gapromptplus_MAE.py', args.experiment_path)
    shutil.copy('models/gapromptplus_PointGPT.py', args.experiment_path)
    shutil.copy('models/PointPrompt.py', args.experiment_path)
    shutil.copy('models/gapromptplus_PointTransformerDINO.py', args.experiment_path)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(tqdm(train_dataloader)):
            ...
            ret = base_model(points, batch_idx=idx)
...
def plot_embedding(data, label, title, category_nums):
    ...
    plt.savefig(TSNE_PATH + "tsne_fix_5.svg")
    return fig
