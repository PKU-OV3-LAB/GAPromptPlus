...
class PointCloudCenterAndNormalize(object):
    def __init__(self, config):
        self.centering = config.get('centering', True)
        self.normalize = config.get('normalize', True)
        self.gravity_dim = config.get('gravity_dim', 2)
        self.append_xyz = config.get('append_xyz', False)
...
class RandomDropout(object):
    def __init__(self, config):
        ...
        """
        self.dropout_ratio = config.get('dropout_ratio', 0.2)
        self.dropout_application_ratio = config.get('dropout_application_ratio', 0.2)
...
class RandomHorizontalFlip(object):
    def __init__(self, config):
        ...
        """
        self.D = 3
        self.upright_axis = {'x': 0, 'y': 1, 'z': 2}[config.get('upright_axis', 'z').lower()]
        # Use the rest of axes for flipping.
        self.horz_axes = set(range(self.D)) - set([self.upright_axis])
        self.aug_prob = config.get('aug_prob', 0.95)
...
class PointCloudScaling(object):
    def __init__(self, config):
        self.scale_min, self.scale_max = np.array(config.get('scale', [2/3, 3/2])).astype(np.float32)
        self.anisotropic = config.get('anisotropic', True)
        self.scale_xyz = config.get('scale_xyz', [True, True, True])
        self.mirror = torch.from_numpy(np.array(config.get('mirror', [0, 0, 0])))
...
class PointCloudTranslation(object):
    def __init__(self, config):
        self.shift = torch.from_numpy(np.array(config.get('shift', [0.2, 0.2, 0]))).to(torch.float32)
...
class PointCloudScaleAndTranslate(object):
    def __init__(self, config):
        ...
        self.scale_xyz = config.get('scale_xyz', [True, True, True])
        self.anisotropic = config.get('anisotropic', True)
        self.shift = torch.from_numpy(np.array(config.get('shift', [0.2, 0.2, 0.2]))).to(torch.float32)
        self.mirror = torch.from_numpy(np.array(config.get('mirror', [0, 0, 0])))
...
class PointCloudJitter(object):
    def __init__(self, config):
        self.noise_std = config.get('jitter_sigma', 0.01)
        self.noise_clip = config.get('jitter_clip', 0.05)
...
class PointCloudRotation(object):
    def __init__(self, config):
        self.angle = np.array(config.get('angle', [0, 0, 0])) * np.pi
...
class ChromaticDropGPU(object):
    def __init__(self, config):
        self.color_drop = config.get('color_drop', 0.2)
...
class ChromaticNormalize(object):
    def __init__(self, config):
        color_mean = config.get('color_mean', [0.5136457, 0.49523646, 0.44921124])
        color_std=config.get('color_std', [0.18308958, 0.18415008, 0.19252081])
