...
class RandomRotate(object):
    def __init__(self, config):
        self.angle = config.get('angle', [0, 0, 1])
...
class RandomRotateZ(object):
    def __init__(self, config):
        self.angle = config.get('angle', 1.0) * np.pi
        rotate_dim = config.get('rotate_dim', 2)
        self.random_rotate = config.get('random_rotate', True)
        axis = np.zeros(3)
        axis[rotate_dim] = 1
        self.axis = axis
...
class RandomScale(object):
    def __init__(self, config):
        self.scale = config.get('scale', [0.8, 1.2])
        self.anisotropic = config.get('scale_anisotropic', False)
        self.scale_xyz = config.get('scale_xyz', [True, True, True])
        self.mirror = np.array(config.get('mirror',[-1, -1, -1]))
...
class RandomScaleAndJitter(object):
    def __init__(self, config):
        self.scale = config.get('scale', [0.8, 1.2])
        self.scale_min, self.scale_max = np.array(self.scale).astype(np.float32)
        self.scale_xyz = config.get('scale_xyz', [True, True, True])
        self.anisotropic = config.get('scale_anisotropic', False)
        self.noise_sigma = False.get('jitter_sigma', 0.01)
...
class RandomShift(object):
    def __init__(self, config):
        self.shift = config.get('shift', [0.2, 0.2, 0])
...
class RandomScaleAndTranslate(object):
    def __init__(self, config):
        self.scale = config.get('scale', [0.9, 1.1])
        self.scale_xyz = config.get('scale_xyz', [1, 1, 1])
        self.shift = config.get('shift', [0.2, 0.2, 0])
...
class RandomJitter(object):
    def __init__(self, config):
        self.noise_sigma = config.get('jitter_sigma', 0.01)
        self.noise_clip = config.get('jitter_clip', 0.05)
...
class ChromaticAutoContrast(object):
    def __init__(self, config):
        self.p = config.get('p', 0.2)
        self.blend_factor = config.get('blend_factor', None)
...
class ChromaticTranslation(object):
    def __init__(self, config):
        self.p = config.get('p', 0.95)
        self.ratio = config.get('ratio', 0.05)
...
class HueSaturationTranslation(object):
    ...
    def __init__(self, config):
        self.hue_max = config.get('hue_max', 0.5)
        self.saturation_max = config.get('saturation_max', 0.2)
...
class RandomDropFeature(object):
    def __init__(self, config):
        self.p = config.get('feature_drop', 0.2)
        self.dim = config.get('drop_dim', [0, 3])
...
class NumpyChromaticNormalize(object):
    def __init__(self, config):
        color_mean = config.get('color_mean', None)
        color_std = config.get('color_std', None)
